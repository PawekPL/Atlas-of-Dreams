...
class Menu(Scene):
    def __init__(self, manager, window):
        ...
        self.newlabel = pyglet.text.Label(
            "New Project",
        ...
        self.loadbutton.set_handler('on_release', self.load)

        self.loadlabel = pyglet.text.Label(
            "Load Project",
        ...
        self.settingsbutton = OneTimeButton(
            self.assetpos[2][0]*self.window.width,
            self.assetpos[2][1] *
            self.window.height,
            self.pressed,
            self.depressed,
        ...
        self.settingsbutton.set_handler('on_release', self.settings)

        self.settingslabel = pyglet.text.Label(
            "Settings",
    ...
    def on_resize(self, manager, w, h):
        """Update the positions of the buttons and labels when the window is resized"""
        ...
        self.settingsbutton.update(
            x=self.assetpos[2][0]*self.window.width,
        ...
        self.logo.update(
            x=self.assetpos[3][0]*self.window.width,
    ...
    def on_mouse_motion(self, manager, x, y, dx, dy):
        """Update the button states when the mouse moves"""
        self.newbutton.on_mouse_motion(x, y, dx, dy)
        self.loadbutton.on_mouse_motion(x, y, dx, dy)
        self.settingsbutton.on_mouse_motion(x, y, dx, dy)

    def on_mouse_press(self, manager, x, y, buttons, modifiers):
        """Update the button states when the mouse is pressed"""
    ...
    def on_mouse_release(self, manager, x, y, buttons, modifiers):
        """Update the button states when the mouse is released"""
    ...
    def new(self):
        self.manager.set_scene("new")

    def load(self):
        self.manager.set_scene("load")
